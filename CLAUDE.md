...
- **A three-branch release pipeline** (`main` → `test` → `release`, see `RELEASING.md`) with two GitHub Actions workflows: `test.yml` (lint + build, runs on every push to `main`/`test` and on PRs into `test`/`release`) and `build.yml` (auto-bumps the patch version, builds Windows/Linux installers, and publishes a permanent versioned GitHub Release on every push to `release`; pushing a `v*` tag directly instead lets you cut a minor/major release).
...
- Promoting to `release` (merge `test` into it) triggers `build.yml`, which auto-bumps `package.json`'s **patch** version, tags it, builds Windows/Linux installers at that version, and publishes a real, permanent GitHub Release — no manual version step needed for the common case.
- For a **minor/major** release instead of the automatic patch, bump it yourself before pushing (`npm version minor` or `major`, then `git push origin release --follow-tags`) — pushing a `v*` tag directly skips the auto-bump and publishes exactly that tag.
